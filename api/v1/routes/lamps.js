...
const LampsModel = requireWrapper('models/lamps/')
const logger = requireWrapper('helpers/logger')
const express = require('express')
const lampsRouter = express.Router({ mergeParams: true })

// logger.info(LampsModel.modelName)

lampsRouter.route('/')
  .get((req, res, next) => {
    LampsModel.find({}, (err, items) => {
  ...
  .post((req, res, next) => {
    const lamp = new LampsModel(req.body)
    lamp.save(err => {
      if (!err) res.status(201).json(lamp)
      else {
        res.status(400).json({ error: err.message, code: 400 })
        logger.error(err)
      }
...
lampsRouter.route('/:id')
  .get((req, res, next) => {
    LampsModel.findOne({ _id: req.params.id }, (err, item) => {
  ...
  .delete((req, res, next) => {
    LampsModel.findOneAndRemove({ _id: req.params.id }, (err, item) => {
  ...
  .put((req, res, next) => {
    ...
    if (req.params.id !== req.body.id) {
      ...
      for (let key in LampsModel.schema.paths) {
        if (!(key in req.body)) {
          if (LampsModel.schema.paths[key].isRequired === true) {
            err[key] = `Request is missing the required field : ${key}`
          }
          if (LampsModel.schema.paths[key].options.default !== undefined) {
            req.body[key] = LampsModel.schema.paths[key].options.default
          }
        }
      }
      if (Object.keys(err).length !== 0) res.status(400).json({ 'error': err, 'code': 400 })
        ...
        LampsModel.findByIdAndUpdate(req.params.id, req.body, { new: true }, (err, item) => {
  ...
  .patch((req, res, next) => {
    if (req.params.id !== req.body.id) {
      ...
      if (req.body.__v !== undefined) delete req.body.__v
      if (req.body.id !== undefined) delete req.body.id
      LampsModel.findByIdAndUpdate(req.params.id, req.body, { upsert: true, new: true }, (err, item) => {
        if (!err) res.status(200).json(item)
